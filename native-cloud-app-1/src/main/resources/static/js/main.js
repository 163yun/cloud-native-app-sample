$(function () {
    ...
    $('#add-profile-form').bootstrapValidator({
        ...
        submitHandler: function (validator, form, submitButton) {
            var screenName = $("#gitHubHandle").val();
            $(".loader").show();
            ...
            $.ajax({
                ...
                error: function (xhr, status) {
                    $(".loader").hide();
                    $("#add-profile-form").data('bootstrapValidator').updateStatus("gitHubHandle", "INVALID", "callback")
                }
            });
        },
        fields: {
            gitHubHandle: {
                validators: {
                    notEmpty: {
                        message: 'A valid GitHub profile handle is required'
                    },
                    callback: {
    ...
    $.ajax({
        url: '/users/search/findAllUsers?skip=0&limit=100',
        type: "get",
        dataType: "json",
        success: function (data) {
            $(".loader").hide();
    ...
    function drawRow(rowData) {
        ...
        row.append($("<td><img src='" + rowData.profileImageUrl + "' class='img-responsive' width='50px'/></td>"));
        row.append($("<td class='hidden-xs'>" + rowData.name + "</td>"));
        row.append($("<td><a href='http://www.github.com/" + rowData.screenName + "' target='_blank'>@" + rowData.screenName + "</a></td>"));
